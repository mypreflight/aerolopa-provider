...
describe('openapi document', () => {
  it('declares every path the server actually serves', async () => {
    ...
    for (const path of documented) {
      expect(
        await statusOf(path === '/seatmap' ? `${path}?slug=x` : path),
      ).not.toBe(404);
    }
  });
  ...
  it('documents each status the server can answer with', async () => {
    const documented = Object.keys(
      openapiDocument.paths['/seatmap'].get.responses,
    );

    expect(documented).toEqual(
      expect.arrayContaining(['200', '400', '404', '405', '500', '502']),
    );
  ...
  it('keeps the operation enum in step with the handler', () => {
    const op = openapiDocument.paths['/seatmap'].get.parameters.find(
  ...
  it('is served over http', async () => {
    ...
    await createApp(fakeClient())(
      { url: '/openapi.json', method: 'GET' } as IncomingMessage,
      response,
    );
