import {
  AerolopaSeatApiInput,
  AerolopaSeatMapApiInput,
  transformSeat,
  transformSeatMap,
} from './seat-map.types';
...
describe('transformSeat', () => {
  ...
  it('reports a missing window as no window status', () => {
    expect(
      transformSeat('30B', { ...seat, window: null }).windowStatus,
    ).toBeNull();
  });
...
describe('transformSeatMap', () => {
  it('flattens the seat dictionary and keeps the canvas', () => {
    ...
    expect(map.assets.seatRects).toBe(
      'https://maptool.example/lh-32n/seat-rects',
    );
  });
